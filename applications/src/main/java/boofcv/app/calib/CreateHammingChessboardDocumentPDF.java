...
public class CreateHammingChessboardDocumentPDF extends CreateFiducialDocumentPDF {
	...
	@Getter HammingChessboardGenerator g;
	...
	public CreateHammingChessboardDocumentPDF(PaperSize paper, Unit units ) {
		super("Hamming Chessboard by BoofCV", paper, units);
	}
	...
	public void render( ConfigHammingChessboard config ) throws IOException {
		this.config = config;
	...
	protected String createMarkerSizeString() {
		...
		return String.format("squares: %s %2s", strSquare, units.getAbbreviation());
	}
}
