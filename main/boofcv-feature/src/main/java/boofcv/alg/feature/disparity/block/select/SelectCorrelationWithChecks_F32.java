		...
		extends SelectDisparityWithChecksWta<float[], DI> implements Compare_F32
{
	...
	public SelectCorrelationWithChecks_F32(int rightToLeftTolerance, double texture, Class<DI> disparityType) {
		super(-1,rightToLeftTolerance,texture,disparityType);
	}

	public SelectCorrelationWithChecks_F32(SelectCorrelationWithChecks_F32<DI> original ) {
	...
	public void process(int row, float[] scores ) {
		...
		for (int col = 0; col < minDisparity; col++) {
			setDisparity(indexDisparity++ , rangeDisparity );
		}
	...
	private int selectRightToLeft( int col , float[] scores ) {
		...
		int localMax = Math.min(imageWidth,col+maxDisparity)-col-minDisparity;
