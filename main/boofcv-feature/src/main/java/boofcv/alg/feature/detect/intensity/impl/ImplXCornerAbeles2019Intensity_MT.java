...
public class ImplXCornerAbeles2019Intensity_MT {
	public static void process(GrayF32 input, GrayF32 intensity) {
		...
		BoofConcurrency.loopFor(radiusA,input.height-radiusA, y->{
			int inputIdx0 = input.startIndex + (y-3)*input.stride + radiusA;
			int inputIdx1 = input.startIndex + (y-2)*input.stride + radiusA;
			int inputIdx2 = input.startIndex + (y-1)*input.stride + radiusA;
			int inputIdx3 = input.startIndex + (y  )*input.stride + radiusA;
			int inputIdx4 = input.startIndex + (y+1)*input.stride + radiusA;
			int inputIdx5 = input.startIndex + (y+2)*input.stride + radiusA;
			int inputIdx6 = input.startIndex + (y+3)*input.stride + radiusA;

			int outputIdx = intensity.startIndex + y*intensity.stride + radiusA;
			for (int x = radiusA; x < width - radiusA; x++) {
