...
public class NonMaxBorderExtractor  {
	 ...
	 */
	public NonMaxBorderExtractor( int minSeparation, float thresh, boolean useStrictRule) {
	...
	public void process(ImageFloat32 intensityImage, QueueCorner detected) {
		if( useStrictRule ) {
			processStrict(intensityImage,detected);
		} else {
			processNoStrict(intensityImage, detected);
		}
	}
	 ...
	 */
	public void processStrict(ImageFloat32 intensityImage, QueueCorner detected) {
		...
		for( int y = borderInput; y < border; y++) {
			...
			for( int x = borderInput; x < imgWidth; x++ , index++ ) {
				...
				if( isMaxStrict(index, x0, y0, x1, y1) ) {
					detected.add(x, y);
				}
			}
		}
	...
	public void processNoStrict(ImageFloat32 intensityImage, QueueCorner detected) {
		int imgWidth = intensityImage.getWidth()-borderInput;
		int imgHeight = intensityImage.getHeight()-borderInput;
		...
		for( int y = borderInput; y < border; y++) {
			...
			for( int x = borderInput; x < imgWidth; x++ , index++ ) {
				...
				if( isMaxNotStrict(index, x0, y0, x1, y1) ) {
					detected.add(x, y);
				}
			}
		}
	 ...
	 */
	private boolean isMaxStrict(int indexCenter, int x0, int y0, int x1, int y1) {
	 ...
	 * Checks to see if the center pixel is the local maximum
	 */
	private boolean isMaxNotStrict(int indexCenter, int x0, int y0, int x1, int y1) {
		float center = inten.data[indexCenter];
