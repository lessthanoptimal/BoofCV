...
public class FactoryCornerDetector {
	...
	GeneralFeatureDetector<T,D> createGeneral( GeneralFeatureIntensity<T,D> intensity ,
											  int minSeparation , float cornerThreshold , int maxFeatures ) {
		// need to set the extractor ignore border to be twice the intensity's ignore border
		// otherwise false positives can be generated by edge conditions
		int extractBorder = 2*intensity.getIgnoreBorder();
		FeatureExtractor extractor = FactoryFeatureFromIntensity.create(minSeparation,cornerThreshold,extractBorder,false,false,false);
