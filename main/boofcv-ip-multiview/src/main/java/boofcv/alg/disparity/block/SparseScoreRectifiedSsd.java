...
/// Computes sparse SSD scores from rectified input images
public interface SparseScoreRectifiedSsd {
			...
			for (int d = 0; d < disparityRange; d++) {
				...
				for (int y = 0; y < blockHeight; y++) {
					...
					for (int x = 0; x < blockWidth; x++) {
						float error = dataLeft[idxLeft++] - dataRight[idxRight++];
						total += error*error;
					}
				}
		...
		protected void scoreDisparity( final int disparityRange, final boolean leftToRight ) {
			...
			for (int d = 0; d < disparityRange; d++) {
				...
				for (int y = 0; y < blockHeight; y++) {
					int idxRight = y*patchCompare.stride + d;
					for (int x = 0; x < blockWidth; x++) {
						int error = (dataLeft[idxLeft++] & 0xFF) - (dataRight[idxRight++] & 0xFF);
						total += error*error;
					}
				}
				int index = leftToRight ? disparityRange - d - 1 : d;
		...
		protected void scoreDisparity( final int disparityRange, final boolean leftToRight ) {
			...
			for (int d = 0; d < disparityRange; d++) {
				...
				for (int y = 0; y < blockHeight; y++) {
					int idxRight = y*patchCompare.stride + d;
					for (int x = 0; x < blockWidth; x++) {
						int error = (dataLeft[idxLeft++] & 0xFFFF) - (dataRight[idxRight++] & 0xFFFF);
						total += error*error;
					}
				}
				int index = leftToRight ? disparityRange - d - 1 : d;
