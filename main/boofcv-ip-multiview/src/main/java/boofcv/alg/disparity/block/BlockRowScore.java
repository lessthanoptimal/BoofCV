...
public interface BlockRowScore<T extends ImageBase<T>, ScoreArray, ImageData> {
	...
	abstract class ArrayS32<T extends ImageBase<T>, ImageData> implements BlockRowScore<T, int[], ImageData> {
		...
		public void scoreRow( int row, ImageData leftRow, ImageData rightRow, int[] scores,
			...
			// Only disparities whose matched column stays inside the image can produce a score
			int dispMin = Math.max(disparityMin, 1 - left.width);
			int dispMax = Math.min(disparityMax, left.width - 1);

			for (int d = dispMin; d <= dispMax; d++) {
				int dispFromMin = d - disparityMin;

				// The block's first left and right columns differ by the disparity, and one of them is always 0.
				int colLeft = Math.max(0, d);
				int colRight = colLeft - d;
				int numCols = left.width - colLeft - colRight;

				// Compute the scores. The score for column 'c' is stored at the absolute column 'c' within the row,
				// so the offset stays in [0,width) even when disparityMin is negative.
				score(leftRow, rightRow, colLeft, colRight, 0, widthAndBorder - colLeft - colRight, elementScore);
				...
				int indexScore = left.width*dispFromMin + colLeft;
				scores[indexScore++] = score;

				// scores for the remaining columns
				for (int col = 0; col < numCols - 1; col++, indexScore++) {
		...
		public void normalizeRegionScores( int row, int[] scores,
										   int disparityMin, int disparityMax, int regionWidth, int regionHeight,
										   int[] scoresNorm ) {
			// Only disparities whose matched column stays inside the image are scored: |d| <= width-1.
			int dispMin = Math.max(disparityMin, 1 - left.width);
			int dispMax = Math.min(disparityMax, left.width - 1);
			...
			for (int d = dispMin; d <= dispMax; d++) {
				...
				int colLeft = Math.max(0, d);
				int colRight = colLeft - d;
				final int numCols = left.width - colLeft - colRight;
				int indexScore = left.width*dispFromMin + colLeft;

				normalizeScore(row, colLeft, colRight, numCols, regionWidth, regionHeight, scores, indexScore, scoresNorm);
			}
		}
	...
	abstract class ArrayF32<T extends ImageBase<T>> implements BlockRowScore<T, float[], float[]> {
		...
		public void scoreRow( int row, float[] leftRow, float[] rightRow, float[] scores,
			...
			for (int d = dispMin; d <= dispMax; d++) {
				...
				// The block's first left and right columns differ by the disparity, and one of them is always 0.
				int colLeft = Math.max(0, d);
				int colRight = colLeft - d; // = max(0,-d)
				int numCols = left.width - colLeft - colRight; // = width - |d|, always >= 1
		...
		public void normalizeRegionScores( int row, float[] scores,
			...
			for (int d = dispMin; d <= dispMax; d++) {
				...
				int colLeft = Math.max(0, d);
				int colRight = colLeft - d; // = max(0,-d)
				final int numCols = left.width - colLeft - colRight; // = width - |d|
				int indexScore = left.width*dispFromMin + colLeft;
