...
public class RefineMetricWorkingGraph implements VerbosePrint {
	 ...
	 */
	public double maxReprojectionErrorPixel = 1e6;
	...
	protected final FastQueue<Point2D_F64> pixelNorms = new FastQueue<>(Point2D_F64::new);
	...
	void createFeatures3D( SceneWorkingGraph graph ) {
		...
		for (int sceneViewIdx = 0; sceneViewIdx < graph.viewList.size(); sceneViewIdx++) {
			...
			final FastArray<PairwiseImageGraph2.View> inlierViews = inliers.views;
			...
			for (int inlierIdx = 0; inlierIdx < numInliers; inlierIdx++) {
				...
				findUnassignedObsAndKnown3D(wview.inliers, inlierIdx);
	...
	void triangulateAndSave( SceneWorkingGraph.InlierInfo inliers, int inlierIdx ) {
		...
		for (int inlierViewIdx = 0; inlierViewIdx < viewIntIds.size; inlierViewIdx++) {
			...
			listPixelToNorm.get(viewID).compute(pixelObserved.x, pixelObserved.y, pixelNorms.get(inlierViewIdx));
		}
...
//		}
	...
	protected boolean refineViews( SceneWorkingGraph graph ) {
		...
		for (int viewIdx = 0; viewIdx < graph.viewList.size(); viewIdx++) {
			SceneWorkingGraph.View wview = graph.viewList.get(viewIdx);
			wview.world_to_view.set(structure.views.get(viewIdx).worldToView);
