...
class TestVisOdomBundleAdjustment {
	...
	void optimize_noise() {
		...
		// first test a negative. If should fail becaues it can't optimize
		alg.bundleAdjustment.configure(1e-6,1e-6,0);
		alg.optimize();
		assertNotEquals(0.0, alg.bundleAdjustment.getFitScore(), 0.001);

		// This should do much better
		alg.bundleAdjustment.configure(1e-6,1e-6,10);
//		alg.bundleAdjustment.setVerbose(System.out,0);
		alg.optimize();
		assertEquals(0.0, alg.bundleAdjustment.getFitScore(), 0.001);
	}
