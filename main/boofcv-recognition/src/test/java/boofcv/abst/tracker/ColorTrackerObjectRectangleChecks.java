 ...
 * @author Peter Abeles
 */
public abstract class ColorTrackerObjectRectangleChecks extends GenericTrackerObjectRectangleChecks<Planar<GrayU8>> {

	Planar<GrayU8> original = new Planar<>(GrayU8.class, width, height, 3);

	boolean multiColor;

	protected ColorTrackerObjectRectangleChecks( boolean multiColor ) {
	...
	private Polygon2D_I32 setPolygon( Quadrilateral_F64 q ) {
		...
		p.vertexes.data[0].setTo((int)q.a.x, (int)q.a.y);
		p.vertexes.data[1].setTo((int)q.b.x, (int)q.b.y);
		p.vertexes.data[2].setTo((int)q.c.x, (int)q.c.y);
		p.vertexes.data[3].setTo((int)q.d.x, (int)q.d.y);
