...
public abstract class DetectCircleGrid<T extends ImageGray<T>> {
	...
	private final InputToBinary<T> inputToBinary;
	...
	private final List<Grid> validGrids = new ArrayList<>();

	// local work space for manipulating the order of points inside a grid
	private final List<EllipseRotated_F64> work = new ArrayList<>();

	// storage for found clusters
	private final List<List<EllipsesIntoClusters.Node>> clusters = new ArrayList<>();
	private final List<List<EllipsesIntoClusters.Node>> clustersPruned = new ArrayList<>();
