...
public class TestAssociatedTupleDN extends StandardStructChecks {
	// Need to override since the default constructor results in an empty list
	@Override
	protected Object createNew( Class<?> type )
			throws NoSuchMethodException, InvocationTargetException, InstantiationException, IllegalAccessException {
		if (type == AssociatedTupleDN.class)
			return new AssociatedTupleDN(3);
		return super.createNew(type);
	}

	@Test void format_Matrix() {
		var p = new AssociatedTupleDN(2);
		p.set(0, 1.234, 2);
		p.set(1, 3, 4);
