...
public class TestPnpRefineWithPixels4D extends BoofStandardJUnit {

	CameraPinholeBrown camera = new CameraPinholeBrown().fsetK(500, 490, 0, 470, 490, 900, 860);

	@Test void general() {
		var alg = new PnPRefineWithPixels4D();
	...
	private void checkSolution( PnPRefineWithPixels4D alg, List<Point3D_F64> worldPts ) {
		...
		var points = new ArrayList<Point2D4D>();
		for (int i = 0; i < worldPts.size(); i++) {
			var p = new Point2D4D();
			var w = worldPts.get(i);
			p.location.setTo(w.x, w.y, w.z, 1.0);
