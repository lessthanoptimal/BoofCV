...
public class TestPointingToProjectedPixelError extends BoofStandardJUnit {
	Point3Transform2_F64 distortion;

	CameraPinhole pinhole = new CameraPinhole(60, 80, 0.01, 200, 150, 400, 300);
	...
	public TestPointingToProjectedPixelError() {
		distortion = new SphereToNarrowPixel_F64(LensDistortionFactory.narrow(pinhole).distort_F64(false, true));
		...
		pix0 = PerspectiveOps.renderPixel(worldToCamera, pinhole, X, null);

		pix1 = pix0.copy();
		pix1.x += 0.2;
		pix1.y += -0.3;

		Point2D_F64 norm = PerspectiveOps.convertPixelToNorm(pinhole, pix1, null);
		pv1.setTo(norm.x, norm.y, 1);
		pv1.divideIP(pv1.norm());
	}

	@Test void usingPoints() {
		var alg = new PointingToProjectedPixelError(distortion);
