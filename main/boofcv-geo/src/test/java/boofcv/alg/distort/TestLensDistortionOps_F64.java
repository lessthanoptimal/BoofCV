...
public class TestLensDistortionOps_F64 {
	...
	private void checkInsideBorder(Affine2D_F64 affine , boolean bound ) {
		...
		if( bound ) {
			found = LensDistortionOps_F64.boundBoxInside(width, height, transform);
		} else {
			found = LensDistortionOps_F64.centerBoxInside(width, height, transform);
		}
		...
		for (int i = 0; i < width; i++) {
			...
			assertTrue(Intersection2D_F64.contains(found,transform.distX,transform.distY));
		...
		for (int i = 0; i < width; i++) {
			...
			assertFalse(Intersection2D_F64.contains(found,transform.distX,transform.distY));
			transform.compute(i,height+1);
			assertFalse(Intersection2D_F64.contains(found,transform.distX,transform.distY));
		}
