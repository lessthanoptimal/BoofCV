 ...
 * @author Peter Abeles
 */
public class CalibPoseAndPointRodriguesJacobianS implements SchurJacobian<DMatrixSparseCSC>
{
	...
	DMatrixSparseTriplet tripletView = new DMatrixSparseTriplet();
	DMatrixSparseTriplet tripletPoint = new DMatrixSparseTriplet();
...
//		System.out.println("Asdads");
											...
											ViewPointObservations obs)
	{
		for( int j = 0; j < obs.points.size; j++ , countPointObs++ ) {
			...
			jacColView = extrinsicParamStart;
...
//			indexY = indexX + numParameters;
	...
	private void addRodriguesJacobian( DMatrixRMaj Rj , Point3D_F64 worldPt )
	{
		...
		tripletView.addItem(jacRowX,jacColView, -zDot_div_z2*cameraPt.x + Rx);
		tripletView.addItem(jacRowY,jacColView++,-zDot_div_z2*cameraPt.y + Ry);
	}
	...
	private void addTranslationJacobian()
	{
		...
		// partial T.x
		tripletView.addItem(jacRowX,jacColView++,divZ);
		// partial T.y
		tripletView.addItem(jacRowY,jacColView++,divZ);
		// partial T.z
		tripletView.addItem(jacRowX,jacColView,-cameraPt.x*divZ2);
		tripletView.addItem(jacRowY,jacColView,-cameraPt.y*divZ2);
		jacColView += 1;
	}
	...
	private void addWorldPointGradient( DMatrixRMaj R ) {
		...
		// partial P.x
		tripletPoint.addItem(jacRowX,jacColPoint, -R.data[6]*divZ2*cameraPt.x + R.data[0]/cameraPt.z);
		tripletPoint.addItem(jacRowY,jacColPoint, -R.data[6]*divZ2*cameraPt.y + R.data[3]/cameraPt.z);
		jacColPoint += 1;
		// partial P.y
		tripletPoint.addItem(jacRowX,jacColPoint, -R.data[7]*divZ2*cameraPt.x + R.data[1]/cameraPt.z);
		tripletPoint.addItem(jacRowY,jacColPoint, -R.data[7]*divZ2*cameraPt.y + R.data[4]/cameraPt.z);
		jacColPoint += 1;
		// partial P.z
		tripletPoint.addItem(jacRowX,jacColPoint, -R.data[8]*divZ2*cameraPt.x + R.data[2]/cameraPt.z);
		tripletPoint.addItem(jacRowY,jacColPoint, -R.data[8]*divZ2*cameraPt.y + R.data[5]/cameraPt.z);
		jacColPoint += 1;
	}
}
