...
public class TestImplEnhanceFilter extends BoofStandardJUnit {
	...
	public void sharpenInner4(ImageGray input , ImageGray output ) {
		...
		if( input.getDataType().isInteger()) {
			...
			ConvolveImageStandard_SB.convolve(EnhanceImageOps.kernelEnhance4_I32,(GrayU8)input,(GrayS16)expected);
			...
			ConvolveImageStandard_SB.convolve(EnhanceImageOps.kernelEnhance4_F32,(GrayF32)input,(GrayF32)expected);
			GPixelMath.boundImage(expected, 0, 255);
		}
	...
	public void sharpenInner8(ImageGray input , ImageGray output ) {
		...
		if( input.getDataType().isInteger()) {
			...
			expected = new GrayS16(input.width,input.height);
			ConvolveImageStandard_SB.convolve(EnhanceImageOps.kernelEnhance8_I32,(GrayU8)input,(GrayS16)expected);
			GPixelMath.boundImage(expected, 0, 255);
		} else {
			BoofTesting.callStaticMethod(ImplEnhanceFilter.class,"sharpenInner8",input,output,0f,255f);

			expected = new GrayF32(input.width,input.height);
			ConvolveImageStandard_SB.convolve(EnhanceImageOps.kernelEnhance8_F32,(GrayF32)input,(GrayF32)expected);
			GPixelMath.boundImage(expected, 0, 255);
		}
