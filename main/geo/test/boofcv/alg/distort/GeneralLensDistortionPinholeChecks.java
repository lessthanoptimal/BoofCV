 ...
 * @author Peter Abeles
 */
public abstract class GeneralLensDistortionPinholeChecks {

	protected float tol_F32 = 1e-3f;
	protected double tol_F64 = 1e-5;

	public abstract LensDistortionPinhole create();

	@Test
	public void forwardsBackwards_F32() {
		LensDistortionPinhole alg = create();

		for (int i = 0; i < 4; i++) {
			boolean inputPixel = i%2 == 0;
			boolean outputPixel = i/2 == 0;

			PointTransform_F32 distort = alg.distort_F32(inputPixel,outputPixel);
			PointTransform_F32 undistort = alg.undistort_F32(outputPixel,inputPixel);
	...
	public void forwardsBackwards_F64() {
		...
		for (int i = 0; i < 4; i++) {
			...
			PointTransform_F64 distort = alg.distort_F64(inputPixel, outputPixel);
			PointTransform_F64 undistort = alg.undistort_F64(outputPixel, inputPixel);
