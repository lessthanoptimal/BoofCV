 ...
 * @author Peter Abeles
 */
public class ResidualsHomographySampson implements EpipolarResiduals {
	...
	public void process(double[] input, double[] output) {
		...
		for( int i = 0; i < obs.size(); i++ ) {
			...
			if( solver.setA(JJ) ) {
				...
				output[index++] = error.data[0];
				output[index++] = error.data[1];
				output[index++] = error.data[2];
				output[index++] = error.data[3];
			} else {
				output[index++] = 0;
				output[index++] = 0;
				output[index++] = 0;
