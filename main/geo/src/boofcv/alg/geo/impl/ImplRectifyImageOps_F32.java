...
public class ImplRectifyImageOps_F32 {
	 ...
	 */
	private static void adjustCalibrated(DenseMatrix32F rectifyLeft, DenseMatrix32F rectifyRight,
										 DenseMatrix32F rectifyK,
		...
		rectifyK.set(K.matrix_F32());
		rectifyLeft.set(K.mult(rL).matrix_F32());
		rectifyRight.set(K.mult(rR).matrix_F32());
	}

	/**
	 * Internal function which applies the rectification adjustment to an uncalibrated stereo pair
	 */
	private static void adjustUncalibrated(DenseMatrix32F rectifyLeft, DenseMatrix32F rectifyRight,
										   RectangleLength2D_F32 bound, float scale) {
		// translation
		float deltaX = -bound.x0*scale;
		float deltaY = -bound.y0*scale;
		...
		rectifyLeft.set(A.mult(rL).matrix_F32());
		rectifyRight.set(A.mult(rR).matrix_F32());
	}
