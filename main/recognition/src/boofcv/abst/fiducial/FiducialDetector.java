...
public interface FiducialDetector<T extends ImageBase>
{
	...
	/**
	 * <p>Computes metrics which represents the fiducial's pose estimate stability given its current observed state.</p>
	 *
	 * <p>
	 * This number is generated by perturbing landmarks by the user provided amount and seeing how it affects
	 * the pose estimate.  The resulting metrics should be considered more qualitative than quantitative
	 * since exactly how this metric is computed isn't specified and can vary depending on target type of
	 * implementation type.  The results could every vary each
	 * time its called for the exact same inputs if a monti-carlo approach is applied.
	 * </p>
	 *
