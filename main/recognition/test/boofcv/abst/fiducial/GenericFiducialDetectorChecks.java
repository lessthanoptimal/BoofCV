		...
		for( ImageType type : types ) {
			...
			if( results.id.length == detector.totalFound()) {
				...
				for (int i = 0; i < detector.totalFound(); i++) {
					...
					assertFalse(MatrixFeatures_R64.isIdentical(pose.getR(), results.pose.get(i).R, 1e-4));
			...
			for (int i = 0; i < detector.totalFound(); i++) {
				...
				assertTrue(MatrixFeatures_R64.isIdentical(pose.getR(),results.pose.get(i).R,1e-8));
			}
		}
	}
	...
	public void checkRemoveIntrinsic() {
		for( ImageType type : types ) {
			...
			for (int i = 0; i < expected.id.length; i++) {
				...
				assertTrue(MatrixFeatures_R64.isIdentical(expected.pose.get(i).getR(), found.pose.get(i).R, 1e-4));
	...
	public void checkSubImage() {
		...
		for( ImageType type : types ) {
			...
			for (int i = 0; i < detector.totalFound(); i++) {
				...
				assertTrue(MatrixFeatures_R64.isIdentical(pose.getR(),foundPose.get(i).R,1e-8));
			}
		}
	}
	...
	public void clearLensDistortion() {
		for( ImageType type : types ) {
			...
			for (int i = 0; i < after.id.length; i++) {
				...
				assertTrue(MatrixFeatures_R64.isIdentical(before.pose.get(i).R,after.pose.get(i).R,1e-8));
