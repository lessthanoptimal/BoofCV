	...
	protected abstract class BaseCompare extends PermuteWaveletCompare {
		...
		public void applyValidation(WaveletDescription<?> desc, ImageBase input, ImageBase output) {
			...
			if( functionName.contains("Inverse")) {
				...
				m = GecvTesting.findMethod(ImplWaveletTransformNaive.class,functionName,border.getClass(),inv.getClass(),input.getClass(),output.getClass());
				args = new Object[]{border,inv,input,output};
			} else {
				Class<?> borderClass = desc.border.getClass();
				Class<?> coefClass = desc.getForward().getClass();
				m = GecvTesting.findMethod(ImplWaveletTransformNaive.class,functionName,borderClass,coefClass,input.getClass(),output.getClass());
