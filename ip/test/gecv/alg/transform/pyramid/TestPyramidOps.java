...
public class TestPyramidOps {
	...
	public void filter() {
		...
		for( int i = 0; i < scales.length; i++ ) {
			...
			GecvTesting.assertEquals(expected,out.getLayer(i),0,1e-4);
		}
	}
	...
	public void gradient() {
		...
		for( int i = 0; i < scales.length; i++ ) {
			...
			GecvTesting.assertEquals(x,outX.getLayer(i),0,1e-4);
			GecvTesting.assertEquals(y,outY.getLayer(i),0,1e-4);
		}
	}
}
